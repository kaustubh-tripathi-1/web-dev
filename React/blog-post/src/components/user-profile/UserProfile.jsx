...
import { openModal, addNotification } from "../../slices/uiSlice";
...
export default function UserProfile() {
    ...
    const { profile, userPosts, loading, error } = useSelector(
        (state) => state.user
    );
    ...
    if (error) {
        return (
            <div className="min-h-full text-center text-red-600 dark:text-red-400">
                {error}
            </div>
        );
    }
